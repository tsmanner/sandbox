...
struct Parity<DataWidth, ParityWidth, DataShift, typename std::enable_if<(DataWidth % ParityWidth == 0)>::type> {
  ...
  // Multi-bit parity implementation. Uses template recursion to eventually call single bit
  ...
  calc(const DataType& data) {
    return (Parity<DataWidth-ParityGroupSize<DataType>::value, ParityWidth-1>::calc(data >> ParityGroupSize<DataType>::value) << 1) | Parity<ParityGroupSize<DataType>::value>::calc(data);
  }
